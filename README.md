...
- `[stt]` — `model` (`small.en` default; `tiny.en` for speed, `medium`/`large-v3` for more accuracy), `device` (`auto` tries CUDA, falls back to CPU int8), `compute_type`, `engine` (`parakeet` default; see below for Whisper).
- **Choosing an STT engine:** Parakeet TDT is the default: lightweight, non-streaming (~1 s latency), self-punctuating, and ideal for the short utterances dictation is mostly made of. **It needs one extra install** — `pip install -r requirements-parakeet.txt` — so if you only ran `requirements.txt`, either do that or switch `[stt].engine` to `"whisper"`. Whisper is the alternative: it transcribes while you dictate (streaming mode) using `large-v3-turbo` for near-large-v3 accuracy at ~4–8× speed, and needs nothing beyond the base requirements. Switch either way via `[stt].engine` or the Settings "STT engine" dropdown. For GPU, `pip install onnxruntime-directml` (in place of `onnxruntime`) — it runs on your GPU via DirectX 12 (no CUDA, no clash with Whisper's CUDA), auto-detected; warm inference is ~0.1 s. CPU works out of the box (~1 s).
